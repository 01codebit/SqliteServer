...
export async function region_route(req, res) {
    ...
    console.error("[region_route] query: \"" + sql + "\"");
    ...
    db.all(sql, [], (err, rows) => {
        ...
        // console.error("[region_route] result: " + JSON.stringify(rows));
        console.error("[region_route] found " + rows.length + " results");
    ...
    db.close((err) => {
        ...
        console.log('[region_route] close the database connection.');
    });
}
