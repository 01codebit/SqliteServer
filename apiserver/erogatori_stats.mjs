...
export async function erogatori_stats_route(req, res) {
    ...
    let db = new sqlite3.Database(DB_PATH, sqlite3.OPEN_READWRITE, (err) => {
        if (err) {
            console.error('[erogatori_stats_route] connection error: ' + err.message);
        }
        else {
            console.log('[erogatori_stats_route] connected to the database.');
        }
    });

    let sql = queryComunale;
    console.error("[erogatori_stats_route] query: \"" + sql + "\"");
    ...
    db.all(sql, [], (err, rows) => {
        ...
        // console.error("[erogatori_stats_route] result: " + JSON.stringify(rows));
        console.error("[erogatori_stats_route] found " + rows.length + " results");
    ...
    db.close((err) => {
        ...
        console.log('[erogatori_stats_route] close the database connection.');
    });
}
